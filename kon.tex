...
$\textcolor{red}{9) \text{ МНК - линейны по} Y \text{?}}$
...
Тогда Что-то тут написано $z = \frac{\hat{\beta}_1 - \beta_1^{0}}{\sigma_{\hat{\beta}_1}} \sim N(0,1)$

В $\sigma_{\hat{\beta}_1}$ входит $\sigma_{\epsilon}^{2}$ и она неизвестна. Заменим оценкой $\hat{\sigma_{\epsilon}}^{2}$

$z = \frac{\hat{\beta}_1 - \beta_1^{0}}{\hat{\sigma}_{\hat{\beta}_{1}}} \sim $ ?

\textbf{Утверждение 2}:
$$\sigma_{\epsilon}^{2} = \frac{RSS}{n - 2} - \text{несмещенная оценка } \sigma_{\epsilon}^{2}$$


\textbf{Утверждение 3}:
$$\sigma_{\epsilon}^{2} = \frac{RSS}{n - k} - \text{общий случай для k регрессоров (включая свободный член)}$$
$n - k$ - число независимых слагаемых $RSS$. Степени свободы

\textbf{Утверждение 4}:
$$\frac{RSS}{\sigma_{\epsilon}^{2}} \sim \chi_{n -k}^{2} \text{, где } k \text{ - количество регрессоров}$$

\textbf{Утверждение 5}: Оценки $\beta_1$ и $\hat{\sigma}_{\epsilon}^2$ независимы
$$\hat{\sigma}_{\hat{\beta}_0^{2}} = \sqrt{\hat{\sigma}_{\epsilon^{2}} \cdot \frac{\sum{X_i^{2}}}{n\sum{X_i^{2}}}}$$ $$\hat{\sigma}_{\hat{\beta}_1^{2}} = \sqrt{\frac{\hat{\sigma}_{\epsilon^{2}}}{\sum{X_i^{2}}}}$$

$\frac{\hat{\beta}_1 - \beta_1^{0}}{\hat{\sigma}_{\hat{\beta}_{1}}} =\frac{\frac{\hat{\beta}_1 - \beta_1^0}{\sigma_{\hat{\beta}_1}}}{\frac{\hat{\sigma}_{\hat{\beta}_1}}{\sigma_{\hat{\beta}_1}}} = $ продолжение следует...
\end{document}